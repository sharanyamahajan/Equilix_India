...
export function AIAvatar({ state = 'idle' }: AIAvatarProps) {
  ...
  const imageVariants = {
    idle: { scale: 1, filter: 'brightness(1)', y: 0, transition: { duration: 2, ease: "easeInOut", yoyo: Infinity } },
    listening: { scale: 1.05, filter: 'brightness(1.05)', y: -5, transition: { duration: 0.5 } },
    talking: { scale: 1.05, filter: 'brightness(1.05)', y: [0, -5, 0], transition: { duration: 1.5, yoyo: Infinity, ease: "easeInOut" } },
